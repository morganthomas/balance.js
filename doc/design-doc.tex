...
Call a path $p \in P$ ``complete'' if and only if $\text{advance}(p) = \emptyset$ ($\emptyset$ is the empty set).

The goal of the path optimization algorithm is to find a complete path $p \in P$ such that $\text{utility}(p)$ is as small as possible. The paths searched are the feasible paths that can be produced by advancing the initial path. In other words, the paths searched are:

\[ \{\text{initial}\} \cup \text{advance}(\text{initial}) \cup \{\text{advance}(a) : a \in \text{advance}(\text{initial})\} \cup ... \]
