...
\item A function $\text{advance} : P \to \mathcal{P}(P)$ from paths to sets of paths. ($\mathcal{P}(P)$ is the set of all subsets of $P$, i.e. the power set of $P$.)
...
Suppose you have a path $p$. How do you compute $\text{advance}(p)$? Let $k$ be the final element of $p$, or let $k = 1$ if $p$ is the empty sequence. Let $c_1,...,c_m = b_k,...,b_n$. $c_1,...,c_m$ is the sequence of boxes in the input sequence that come after the final break point in $p$. If $b_{k-1}$ exists and has a post-break box, let $d$ be its post-break box.
...
If the line contains only rigid boxes, then $\text{utility}(p)$ is equal to the difference between the length of the line and the sum of the lengths of the boxes.

If the line contains a fill box, then $\text{utility}(p) = 0$.

If the line contains no fill box and contains elastic boxes, then $\text{utility}(p) = r$, where $r$ is the stretch/shrink ratio of the elastic boxes. $r$ is computed as follows.
...
To produce a paragraph velement, you need to provide as input a sequence of paragraph boxes, $b_1,...,b_n$. A paragraph box is very much like the boxes used in line packing, but it is not the same thing. A paragraph box is an object with the following properties:
...
A paragraph velement is a type of box velement. 
