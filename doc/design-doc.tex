...
It is a fact not yet noted that $j$ itself may not exist. If $j$ does not exist, let $\text{advance}(p) = \{p\}$. If $j$ does exist, let $\text{advance}(p) = \{p \smallfrown j, p \smallfrown j^+, \smallfrown j^-\}$; this is a set of one to three paths, depending which of $j^+$ and $j^-$ exist. $p \smallfrown a$ is the path resulting from adding $a$ to the end of $p$.
